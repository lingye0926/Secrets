...
if __name__ == '__main__':
    header = gettablelabel('MVKE', ['MATNR', 'VMSTA', 'PMATN'])
    newsapconn = Callse16()
    newsapconn.logon()
    newsapconn.addMF()
    newsapconn.addtable('MVKE')
    newsapconn.addfields(['MATNR', 'VMSTA', 'PMATN'])
    newsapconn.addquery("VKORG = '1390'")
    # newsapconn.addquery("VMSTA = 'ZC'")
    data = newsapconn.excute()
    fulldata = (header,) + data
    soldtoData = []
with codecs.open(r'C:\Users\cwang2\Documents\B2B Integration\Data\latest\dchain.dat', 'w', 'utf-16') as f:
